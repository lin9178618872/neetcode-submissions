...
class Solution:
    def minWindow(self, s: str, t: str) -> str:
        # 1. need: 存储 t 中字符及其所需数量（即目标需求）
        need = Counter(t)
        
        # 2. window: 存储当前窗口 [left, right) 中字符的出现次数
        window = {}
        
        # required_count: t 中不同字符的种类数量
        required_count = len(need)
        
        # valid: 窗口中满足 need 要求的不同字符的数量
        valid = 0
        ...
        while right < len(s):
            ...
            if c in need:
                ...
                # 如果 c 的数量首次达到 need[c]，则 valid + 1
                if window[c] == need[c]:
                    valid += 1

            # 判断左侧窗口是否要收缩 (window need shrink)
