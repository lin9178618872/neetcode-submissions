...
#合并区间，用0组跟1...n一个一个比较
        ...
        for start, end in intervals[1:]:
            ...
            if start <= last_end:#[0,1],[3,4]->3<1,不是就插入[3,4]。（1],[3）
                merged[-1][1] = max(last_end, end)#换一个，如果成立1<4=4->变成[0,4]。如果上面一行还继续大，就叫第二个数的第二个来跟他比谁大
            else:
...
#找出了空隙区间
        ...
        for i in range(1, len(merged)):#这里的merged默认是有0的，但是要从1开始
            prev_end = merged[i - 1][1]#[0,1][1,2]=1
            curr_start = merged[i][0]#[1,2]=1
